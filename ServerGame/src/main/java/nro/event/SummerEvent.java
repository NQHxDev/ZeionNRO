...
public class SummerEvent extends Event {
            ...
            public void openBaseMenu(Player player) {
               if (canOpenNpc(player)) {
                  this.createOtherMenu(player, ConstNpc.BASE_MENU,
                        "Xin chào, sự kiện hè 2024 đang được diễn ra\n PHÁ TAN CƠN NÓNG MÙA HÈ, Chúc các cư dân vui vẻ.",
                        "Đổi Quà\nSự Kiện", "Tắm\nNước Nóng", "Bắt\nSâu Bọ", "Từ chối");
               }
            }
            ...
            public void confirmMenu(Player player, int select) {
               ...
               if (player.iDMark.isBaseMenu()) {
                  switch (select) {
                     case 0:
                        this.createOtherMenu(player, ConstNpc.ORTHER_MENU,
                              "Xin chào, sự kiện hè 2024 đang được diễn ra", "Đổi\n Vỏ Ốc", "Đổi Sò", "Đổi Cua",
                              "Đổi \nSao Biển", "Đổi Quà\n Đặc Biệt", "Từ Chối");
                        break;
                     case 1:
                        this.createOtherMenu(player, ConstNpc.ORTHER_MENU1,
                              "Xin chào, sự kiện hè 2024 đang được diễn ra", "Chế Tạo Bồn\n Tắm Gỗ",
                              "Chế Tạo Bồn\n Tắm Vàng", "Từ chối");
                        break;
                     case 2:
                        this.createOtherMenu(player, ConstNpc.ORTHER_MENU2,
                              "Xin chào, sự kiện hè 2024 đang được diễn ra", "Tặng Bọ\nCánh Cứng", "Tặng\nNgài Đêm",
                              "Từ chối");
                        break;
                  }
                  ...
                  if (rw.template.type == 11) {// đồ đeo lưng
                     ...
                     if (rw.template.id != ConstItem.MANH_AO || rw.template.id != ConstItem.MANH_QUAN
                           || rw.template.id != ConstItem.MANH_GIAY) {
                  ...
                  sb.append("|2|Chế tạo Bồn Tắm ").append(select == 1 ? "gỗ" : "vàng\n").append("\n|1|Cành khô: ")
                        .append(InventoryService.gI().getQuantity(player, ConstItem.CANH_KHO)).append("/50\n")
                        .append("Nước suối tinh khiết: ")
                        .append(InventoryService.gI().getQuantity(player, ConstItem.NUOC_SUOI_TINH_KHIET))
                        .append("/20\n").append("Gỗ lớn: ")
                        .append(InventoryService.gI().getQuantity(player, ConstItem.GO_LON)).append("/20\n")
                        .append("Que đốt: ").append(InventoryService.gI().getQuantity(player, ConstItem.QUE_DOT))
                        .append("/2\n");
                  ...
                  switch (select) {
                     case 0: {
                        ...
                        RandomCollection<Integer> rd = new RandomCollection<Integer>();
   ...
   public void dropItem(Player player, Mob mob, List<ItemMap> list, int x, int yEnd) {
      ...
      if (Util.isTrue(1, 50)) {
         ...
         } else if (mapID == ConstMap.RUNG_BAMBOO
               || mapID == ConstMap.RUNG_DUONG_XI && rwLimit[ConstRewardLimit.BO_KIEN_VUONG_HAI_SUNG] < 100) {
            rwLimit[ConstRewardLimit.BO_KIEN_VUONG_HAI_SUNG]++;
            itemID = ConstItem.BO_KIEN_VUONG_HAI_SUNG;
         } else if (mapID == ConstMap.RUNG_NGUYEN_SINH
               || mapID == ConstMap.RUNG_THONG_XAYDA && rwLimit[ConstRewardLimit.BO_HUNG_TE_GIAC] < 100) {
            rwLimit[ConstRewardLimit.BO_HUNG_TE_GIAC]++;
            itemID = ConstItem.BO_HUNG_TE_GIAC;
         } else if (mapID == ConstMap.DOI_NAM_TIM
               || mapID == ConstMap.THUNG_LUNG_NAMEC && rwLimit[ConstRewardLimit.BO_KEP_KIM] < 100) {
   ...
   private void insectTrapping(Player player, Item item) {
      ...
      if (mapID == ConstMap.DOI_NAM_TIM || mapID == ConstMap.THUNG_LUNG_NAMEC || mapID == ConstMap.RUNG_THONG_XAYDA
            || mapID == ConstMap.RUNG_BAMBOO || mapID == ConstMap.RUNG_DUONG_XI) {
         flag = true;
      }
      StringBuilder sb = new StringBuilder();
      ...
      sb.append("|1|Làm mồi nhử ").append(itemID == ConstItem.HU_MAT_ONG ? "Hũ mật ong \n" : "Vợt bắt bọ \n")
            .append("|").append(nBoKienVuong < 10 ? ConstTextColor.BLUE : ConstTextColor.RED).append("|")
            .append("Bọ Kiến Vương: ").append(nBoKienVuong).append("/10\n")
            .append("|").append(nBoHung < 10 ? ConstTextColor.BLUE : ConstTextColor.RED).append("|")
            .append("Bọ Hung Tê Giác: ").append(nBoHung).append("/10\n")
            .append("|").append(nBoKepKim < 10 ? ConstTextColor.BLUE : ConstTextColor.RED).append("|")
            .append("Bọ Kẹp Kìm: ").append(nBoKepKim).append("/10\n")
            .append("|").append(flag ? ConstTextColor.BLUE : ConstTextColor.RED).append("|")
            .append("Đồi nấm tím, Thung lũng Namec,Rừng nguyên sinh, Rừng thông Xayda ,Rừng Bamboo, Rừng dương xỉ");
      if (!flag) {
