 ...
 * Nếu logic nặng (scan list lớn, AI phức tạp, I/O...), hãy đẩy sang
 * GameScheduler.SCHED.execute(..)
 * để tránh làm chậm GameLoop.
 *
 * - periodMs() là chu kỳ tick mong muốn (ms).
 * GameLoop sẽ gọi tick() theo period này, nhưng vẫn có giới hạn ngân sách mỗi
 * frame.
 *
 * - isActive() cho phép giảm tần số tick khi thực thể “rảnh” (vd: map không có
 * player).
 */
public interface Tickable {
    ...
    *
    * @param nowMillis thời gian hiện tại (System.currentTimeMillis()) do GameLoop
    *                  truyền vào.
