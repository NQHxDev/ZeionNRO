...
public class CombineServiceNew {
      ...
      switch (player.combineNew.typeCombine) {
            ...
            if (player.combineNew.itemsCombine.size() == 1) {
               ...
               if (isTrangBiPhaLeHoa(item)) {
                  ...
                  if (star < MAX_STAR_ITEM) {
                     ...
                     player.combineNew.ratioCombine = Manager.TILE_NCAP == 0 ? getRatioPhaLeHoa(star)
                           : Manager.TILE_NCAP;
            ...
            if (player.combineNew.itemsCombine.size() == 1) {
               ...
               if (isTrangBiPhaLeHoa(item)) {
                  ...
                  if (star < MAX_STAR_ITEM) {
                     ...
                     int need = player.combineNew.goldCombine; // tái dùng 'goldCombine' làm "số lượng" thỏi/xu ở mốc
                                                               // đặc biệt
                     if (star <= 10) {
                     ...
                     this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Vật phẩm đã đạt tối đa sao pha lê",
                           "Đóng");
                  }
               } else {
         ...
         case NHAP_NGOC_RONG:
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
               if (player.combineNew.itemsCombine.size() == 1) {
                  ...
                  if (item != null) {
                     int soluong = 7;
                     if (item.isNotNullItem() && (item.template.id > 14 && item.template.id <= 20)
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Không có ép lên được nữa !!!",
                              "Đóng");
                     }
                  }
               } else {
                  this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Cần 7 viên ngọc rồng cùng sao trở lên",
                        "Đóng");
               }
            } else {
               ...
               if (player.combineNew.itemsCombine.size() == 2) {
                  ...
                  if (isTrangBiAn(item)) {
                     if (item != null && item.isNotNullItem() && dangusac != null
                        ...
                        this.longnu.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn chưa bỏ đủ vật phẩm !!!",
                              "Đóng");
                     }
                  } else {
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
               if (player.combineNew.itemsCombine.size() == 2) {
                  ...
                  if (isTrangBiTaySPL(item)) {
                     ...
                     this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                           "Vật phẩm này không thể Tẩy\nVui lòng chỉ chọn 1 món (Áo, quần, giày, găng hoặc rada)",
                           "Đóng");
                  }
               } else {
            ...
            if (player.combineNew.itemsCombine.size() >= 2 && player.combineNew.itemsCombine.size() < 4) {
               ...
               if (isCoupleItemNangCapCheck(itemDo, itemDNC)) {
                  ...
                  if (level < MAX_LEVEL_ITEM) {
                     ...
                     player.combineNew.ratioCombine = Manager.TILE_NCAP == 0 ? (float) getTileNangCapDo(level)
                           : Manager.TILE_NCAP;
                     ...
                     this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Trang bị của ngươi đã đạt cấp tối đa",
                           "Đóng");
                  }
               } else {
                  this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
               ...
               this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy chọn 1 trang bị và 1 loại đá nâng cấp",
                     "Đóng");
            }
            break;
         case NANG_CAP_CHAN_MENH:
            if (player.combineNew.itemsCombine.size() == 2) {
               ...
               player.combineNew.TileNangcap = Manager.TILE_NCAP == 0 ? getTiLeNangcapChanmenh(star)
                     : Manager.TILE_NCAP;
               if (bongTai != null && manhVo != null && (bongTai.template.id >= 1300 && bongTai.template.id < 1308)) {
                  ...
                  if (player.combineNew.DiemNangcap <= player.inventory.ruby) {
                     ...
                     npcSay += "Còn thiếu " + Util.numberToMoney(player.combineNew.DiemNangcap - player.inventory.ruby)
                           + " Hồng ngọc";
                     baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, npcSay, "Đóng");
                  }
               } else {
         ...
         case NANG_CAP_BONG_TAI: {
            ...
            // Check đã có bông tai cấp tiếp theo trong hành trang chưa (để đồng bộ với hàm
            // nâng)
            ...
            if (nextTemplateId != -1) {
               ...
               if (findItemBag != null) {
                  baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                        "Ngươi đã có " + getPorataNameByLevel(nextLevel)
                              + " trong hành trang rồi\nKhông thể nâng cấp nữa.",
                        "Đóng");
                  break;
               }
            }
            ...
            if (enoughGold && enoughRuby) {
               ...
               if (!enoughGold) {
                  npcSay += "|7|Thiếu " + Util.numberToMoney(player.combineNew.goldCombine - player.inventory.gold)
                        + " vàng\n";
               }
               if (!enoughRuby) {
                  npcSay += "|7|Thiếu " + Util.numberToMoney(player.combineNew.gemCombine - player.inventory.ruby)
                        + " Hồng ngọc\n";
               }
               baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, npcSay, "Đóng");
            }
         ...
         case CHUYEN_HOA_DO_HUY_DIET:
            ...
            if (player.combineNew.itemsCombine.size() == 1) {
               ...
               if (huydietok == 0) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta chỉ có thể chuyển hóa đồ Hủy diệt thôi",
                        "Đóng");
                  return;
               }
               ...
               this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                     "Ta chỉ có thể chuyển hóa 1 lần 1 món đồ Hủy diệt", "Đóng");
            }
            break;
         case CHE_TAO_THANH_TON:
            ...
            if (player.combineNew.itemsCombine.size() == 3) {
               if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSachCoNgu())
                     .count() < 1) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu sách cổ ngữ", "Đóng");
                  return;
               }
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.template.id == 1687).count() < 1) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu long huyết thạch", "Đóng");
                  return;
               }
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.isNguyenTo() && item.quantity >= 999).count() < 1) {
               ...
               if (player.combineNew.itemsCombine.size() > 3) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Tính hối lộ ta hay gì, hãy gọi ta là liêm",
                        "Đóng");
                  return;
               }
               this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Chưa đủ nguyên liệu chế tạo mẹ gì, cook",
                     "Đóng");
            }
            break;
         case NANG_SKH_THANH_TON:
            ...
            if (player.combineNew.itemsCombine.size() == 3) {
               if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isThanhTon())
                     .count() < 1) {
                  this.hangNga.createOtherMenu(player, ConstNpc.IGNORE_MENU, "THIẾU THẦN LONG THẠCH", "Đóng");
                  return;
               }
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.isSKH() && item.isDTS()).count() < 1) {
                  this.hangNga.createOtherMenu(player, ConstNpc.IGNORE_MENU, "THIẾU TRANG BỊ SKH THIÊN SỨ", "Đóng");
                  return;
               }
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.isThanLongThach()).count() < 1) {
         ...
         case THU_HOI_DO_TL:
            if (player.combineNew.itemsCombine.size() == 0) {
               this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy bỏ trang bị con muốn thu hồi vào",
                     "Đóng");
               return;
            }
            if (player.combineNew.itemsCombine.size() == 1) {
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.isTrangBiThuHoi() && item.quantity >= 1)
                     .count() < 1) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Đây không phải đồ TL, HD, TS , Thánh Tôn",
                        "Đóng");
                  return;
               }
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.isSKH() && item.quantity >= 1).count() == 1) {
               ...
               if (player.combineNew.itemsCombine.size() > 1) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta chỉ hỗ trợ đổi mỗi lần một món",
                        "Đóng");
                  return;
               }
               this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Mấy thứ vớ vẩn này thu hồi mẹ gì, cook",
                     "Đóng");
            }
            break;
         case PHAN_RA_DO_TS:
            ...
            if (player.combineNew.itemsCombine.size() == 1) {
               ...
               if (dothiensu == 0) {
                  this.npcwhists.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                        "Ta chỉ có thể chuyển hóa đồ Thiên sứ thôi", "Đóng");
                  return;
               }
               String npcSay = "|2|Sau khi chuyển hóa vật phẩm\n|7|"
         ...
         case NANG_CAP_DO_TS:
            ...
            if (player.combineNew.itemsCombine.size() == 3) {
               if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isCongThuc())
                     .count() < 1) {
                  this.npcwhists.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu mảnh Công thức", "Đóng");
                  return;
               }
               if (player.combineNew.itemsCombine.stream()
                     .filter(item -> item.isNotNullItem() && item.template.id == 1083).count() < 1) {
               ...
               String npcSay = "|2|Con có muốn đổi các món nguyên liệu ?\n|7|"
                     + "Và nhận được "
                     + player.combineNew.itemsCombine.stream().filter(Item::isManhTS).findFirst().get().typeNameManh()
                     + " thiên sứ tương ứng\n"
                     + "|1|Cần " + Util.numberToMoney(COST) + " vàng";
         ...
         case NANG_CAP_SKH_VAI_THO:
            ...
            if (player.combineNew.itemsCombine.size() == 3) {
               ...
               if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSKH())
                     .count() >= 1) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Đây là đồ skh, không thể nâng cấp",
                        "Đóng");
                  return;
               }
               String npcSay = "|2|Con có muốn đổi các món nguyên liệu ?\n|7|"
         ...
         case NANG_CAP_TRANG_BI:
            if (player.combineNew.itemsCombine.size() == 0) {
               this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                     "Hãy đưa ta 1 món đồ SKH hoặc Phụng Thiên Kích\n"
                           + "Và đá cường hoá để có thể nâng cấp trang bị thêm 1 bậc",
                     "Đóng");
               return;
            }
            if (player.combineNew.itemsCombine.size() == 2) {
               ...
               if (player.inventory.gold < 200000000) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Cần có đủ 2 tỏi vàng mới có thể nâng cấp",
                        "Đóng");
                  return;
               }
               ...
               if (player.combineNew.itemsCombine.size() > 2) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Chỉ cần một món đồ và đá cường hoá thôi",
                        "Đóng");
                  return;
               }
               this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU,
         ...
         case NANG_CAP_THAN_LINH:
            ...
            if (player.combineNew.itemsCombine.size() == 1) {
               ...
               if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSKH())
                     .count() >= 1) {
                  this.thoren.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Đây là đồ skh chỉ có thể cường hoá",
                        "Đóng");
                  return;
               }
               Item doThanLinh = player.combineNew.itemsCombine.get(0);
               String npcSay = "|2|Con có muốn nâng cấp " + doThanLinh.template.name + " Thành" + "\n|7|"
                     + doThanLinh.typeName() + " Hủy diệt "
                     + Service.getInstance().get_HanhTinh(doThanLinh.template.gender) + "\n|0|"
         ...
         case GIA_HAN_VAT_PHAM:
            if (player.combineNew.itemsCombine.size() == 2) {
               ...
               if (player.inventory.gold > 200000000) {
                  ...
                  this.xetank.createOtherMenu(player, ConstNpc.IGNORE_MENU,
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
               if (player.combineNew.itemsCombine.size() == 2) {
                  ...
                  if (isTrangBiPhapsu(item)) {
                     if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()
                           && dangusac.template.id == 1235 && dangusac.quantity >= 1) {
                        String npcSay = item.template.name + "\n|2|";
                        for (ItemOption io : item.itemOptions) {
                           npcSay += io.getOptionString() + "\n";
                        }
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn chưa bỏ đủ vật phẩm !!!",
                              "Đóng");
                     }
                  } else {
                     this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Vật phẩm này không thể Pháp sư hóa",
                           "Đóng");
                  }
               } else {
                  this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Cần bỏ đủ vật phẩm yêu cầu", "Đóng");
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
               if (player.combineNew.itemsCombine.size() == 2) {
                  ...
                  if (isTrangBiLinhHoa(item)) {
                     if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()
                           && dangusac.template.id == 698 && dangusac.quantity >= 1) {
                        String npcSay = item.template.name + "\n|2|";
                        for (ItemOption io : item.itemOptions) {
                           npcSay += io.getOptionString() + "\n";
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE, npcSay, "Linh Hoá",
                              "Từ chối");
                     } else {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn chưa bỏ đủ vật phẩm !!!",
                              "Đóng");
         ...
         case TAY_PHAP_SU:
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
               if (player.combineNew.itemsCombine.size() == 2) {
                  ...
                  if (isTrangBiPhapsu(item)) {
                     if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()
                           && dangusac.template.id == 1236 && dangusac.quantity >= 1) {
                        String npcSay = item.template.name + "\n|2|";
                        for (ItemOption io : item.itemOptions) {
                           npcSay += io.getOptionString() + "\n";
                        }
                     ...
                     this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Vật phẩm này không thể thực hiện",
                           "Đóng");
                  }
               } else {
                  this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Cần bỏ đủ vật phẩm yêu cầu", "Đóng");
         ...
         case NANG_CAP_SACH_TUYET_KY:
            if (player.combineNew.itemsCombine.size() == 2) {
               ...
               for (Item item : player.combineNew.itemsCombine) {
                  if (issachTuyetKy(item)
                        && (item.template.id == 1510 || item.template.id == 1512 || item.template.id == 1514)) {
                     sachTuyetKy = item;
   ...
   public void openThuHoiTrangBi(Player player) {
      ...
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         Item trangbi = player.combineNew.itemsCombine.stream()
               .filter(item -> item.isNotNullItem() && item.isTrangBiThuHoi() && item.quantity >= 1).findFirst().get();
   ...
   public void openSKHts(Player player) {
      ...
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         ...
         List<Item> itemDTS = player.combineNew.itemsCombine.stream()
               .filter(item -> item.isNotNullItem() && item.isDTS()).collect(Collectors.toList());
         ...
         if (new Item(itemId).isDTL()) {
            ...
            item.itemOptions.remove(
                  item.itemOptions.stream().filter(itemOption -> itemOption.optionTemplate.id == 21).findFirst().get());
   ...
   public void openDTS(Player player) {
      ...
      Item itemHDs = player.combineNew.itemsCombine.stream()
            .filter(item -> item.isNotNullItem() && item.template.id == 1083).findFirst().get();
   ...
   public void openCheTaoThanhTon(Player player) {
      ...
      Item itemTL = player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSachCoNgu())
            .findFirst().get();
      Item itemHDs = player.combineNew.itemsCombine.stream()
            .filter(item -> item.isNotNullItem() && item.template.id == 1687).findFirst().get();
      Item itemManh = player.combineNew.itemsCombine.stream()
            .filter(item -> item.isNotNullItem() && item.isNguyenTo() && item.quantity >= 999).findFirst().get();
      ...
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         ...
         if (new Item(itemId).isThanhTon()) {
            ...
            // items.itemOptions.add(new
            // ItemOption(ItemService.gI().optionIdSKHThanhTon(skhId), 1));
            items.itemOptions.remove(items.itemOptions.stream().filter(itemOption -> itemOption.optionTemplate.id == 21)
                  .findFirst().get());
   ...
   public void openCheTaoSKHThanhTon(Player player) {
      ...
      if (player.combineNew.itemsCombine.size() != 3) {
         Service.getInstance().sendThongBao(player,
               "Vui lòng đặt đúng 3 món: 1 Thánh Tôn + 1 SKH Thiên Sứ + 1 Thần Long Thạch");
         return;
      }
   ...
   public void openSKHVaiTho(Player player) {
      ...
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         ...
         List<Item> itemDHD = player.combineNew.itemsCombine.stream()
   ...
   public void NangCapKimDan(Player player) {
      ...
      if (daCuongHoa.quantity < soLuongDaCan) {
         Service.getInstance().sendThongBaoOK(player,
               "Thiếu đá cường hoá, ở cấp này bạn cần " + soLuongDaCan + " Đá Cướng Hoá");
         return;
      }
      if (levelDa >= 18) {
         Service.getInstance().sendThongBao(player, "|7|Bạn đã nâng cấp tới cấp độ cao nhất");
   ...
   private void GiaHanTrangBi(Player player) {
      ...
      if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.id == 1346)
            .count() != 1) {
         Service.getInstance().sendThongBao(player, "Thiếu Bùa Gia Hạn");
         return;
      }
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         Item thegh = player.combineNew.itemsCombine.stream().filter(item -> item.template.id == 1346).findFirst()
               .get();
   ...
   private void anTrangBi(Player player) {
      ...
      int optionIdToAdd = switch (dangusac.template.id) {
         case 1232 -> 34;
         case 1233 -> 35;
         case 1234 -> 36;
         case 1689 -> 37;
      ...
      if (optionIdToAdd == -1 || dangusac.quantity < 99) {
         Service.getInstance().sendThongBao(player, "Nguyên liệu không đủ (99 viên) hoặc không hợp lệ");
         return;
      }

      // Kiểm tra xem trang bị đã có bất kỳ "Ấn" nào trong dải 34-39 chưa
      boolean hasAn = item.itemOptions.stream()
            .anyMatch(io -> io.optionTemplate.id >= 34 && io.optionTemplate.id <= 39);

      if (!hasAn) {
         item.itemOptions.add(new ItemOption(optionIdToAdd, 1));
         ...
         Service.getInstance().sendThongBao(player, "Trang bị của bạn đã có ấn rồi!");
      }
   }
   ...
   private void longAnTrangBi(Player player) {
      ...
      // Đồng nhất số lượng cần thiết là 555 viên theo logic cũ
      if (optionIdToAdd == -1 || dangusac.quantity < 555) {
         Service.getInstance().sendThongBao(player, "Nguyên liệu không đủ (555 viên) hoặc không hợp lệ");
         return;
      }

      // Kiểm tra xem trang bị đã có "Ấn" thường (34-36) chưa
      boolean hasNormalAn = item.itemOptions.stream()
            .anyMatch(io -> io.optionTemplate.id >= 34 && io.optionTemplate.id <= 36);

      // Kiểm tra xem đã có "Long Ấn" (37-39) chưa
      boolean hasLongAn = item.itemOptions.stream()
            .anyMatch(io -> io.optionTemplate.id >= 37 && io.optionTemplate.id <= 39);

      if (hasNormalAn && !hasLongAn) {
         item.itemOptions.add(new ItemOption(optionIdToAdd, 1));
         ...
         Service.getInstance().sendThongBao(player, "Trang bị của bạn chưa được nâng cấp Ấn thường!!!");
      } else {
         Service.getInstance().sendThongBao(player, "Trang bị của bạn đã có Long Ấn rồi!");
      }
   }
   ...
   private void TaySaoPhaLe(Player player) {
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         if (!player.combineNew.itemsCombine.isEmpty()) {
            ...
            if (item.isNotNullItem() && thoivang.isNotNullItem() && (thoivang.template.id == 457)
   ...
   private void phucHoiSach(Player player) {
      if (player.combineNew.itemsCombine.size() != 1) {
         Service.getInstance().sendThongBao(player, "Chỉ được bỏ duy nhất 1 cuốn sách tuyệt kỹ vào ô kết hợp.");
         return;
      }

      Item sachTuyetKy = player.combineNew.itemsCombine.get(0);
      if (sachTuyetKy == null || !issachTuyetKy(sachTuyetKy)) {
         Service.getInstance().sendThongBao(player, "Vật phẩm này không phải là Sách Tuyệt Kỹ!");
         return;
      }

      int goldRequired = 10_000_000;
      int materialRequired = 10;
      Item cuonSachCu = InventoryService.gI().findItemBagByTemp(player, (short) 1509);

      if (cuonSachCu == null || cuonSachCu.quantity < materialRequired) {
         Service.getInstance().sendThongBao(player, "Bạn cần có ít nhất 10 Cuốn sách cũ trong hành trang.");
         return;
      }

      if (player.inventory.gold < goldRequired) {
         Service.getInstance().sendThongBao(player, "Bạn không đủ vàng để thực hiện phục hồi (Cần 10Tr Vàng).");
         return;
      }

      ItemOption durabilityOption = null;
      ...
      if (durabilityOption == null) {
         Service.getInstance().sendThongBao(player, "Sách này không có chỉ số độ bền để phục hồi.");
         return;
      }

      if (durabilityOption.param >= 1000) {
         Service.getInstance().sendThongBao(player, "Sách của bạn vẫn còn đủ độ bền, không cần phục hồi.");
         return;
      }

      // Xử lý trừ tiền và vật phẩm
      player.inventory.gold -= goldRequired;
      InventoryService.gI().subQuantityItemsBag(player, cuonSachCu, materialRequired);

      // Phục hồi độ bền
      durabilityOption.param = 1000;

      InventoryService.gI().sendItemBags(player);
      Service.getInstance().sendMoney(player);
      sendEffectSuccessCombine(player);
      reOpenItemCombine(player);
      Service.getInstance().sendThongBao(player, "Đã phục hồi độ bền sách thành công!");
   }

   private void phanRaSach(Player player) {
      if (player.combineNew.itemsCombine.size() != 1) {
         Service.getInstance().sendThongBao(player, "Chỉ được bỏ duy nhất 1 cuốn sách tuyệt kỹ để phân rã.");
         return;
      }
      ...
      int goldRequired = 10_000_000;
      if (player.inventory.gold < goldRequired) {
         Service.getInstance().sendThongBao(player, "Bạn không đủ vàng để phân rã (Cần 10Tr Vàng).");
         return;
      }
      ...
      if (luotTay > 0) {
         Service.getInstance().sendThongBao(player, "Sách vẫn còn lượt sử dụng, không thể phân rã!");
         return;
      }

      if (InventoryService.gI().getCountEmptyBag(player) < 1) {
         Service.getInstance().sendThongBao(player, "Hành trang của bạn đã đầy!");
         return;
      }
      ...
      sendEffectSuccessCombine(player);
      reOpenItemCombine(player);
      Service.getInstance().sendThongBao(player, "Phân rã sách thành công, nhận được 5 Cuốn sách cũ!");
   }
   ...
   // if (!player.combineNew.itemsCombine.isEmpty()) {
   // Item item = player.combineNew.itemsCombine.get(0);
   // if (item != null && item.isNotNullItem() && (item.template.id > 0 &&
   // item.template.id <= 3) && item.quantity >= 1) {
               ...
               if (lvcheck < 12) {
                  if (optionStar == null) {
                     ...
                     for (ItemOption ioo : item.itemOptions) {
                        ...
                        if ((ioo.optionTemplate.id == 229 || ioo.optionTemplate.id == 230
                              || ioo.optionTemplate.id == 231 || ioo.optionTemplate.id == 232)
                              && (ioo.optionTemplate.id == check)) {
                           if (check == 232) {
                              ioo.param += 1;
                           } else {
                              ioo.param += 2;
   ...
   private void linhHoaTrangBi(Player player) {
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         if (!player.combineNew.itemsCombine.isEmpty()) {
            ...
            if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()
                  && (dangusac.template.id == 698) && dangusac.quantity >= lvcheck) {
               if (lvcheck < 99) {
                  if (optionStar == null) {
                     ...
                     for (ItemOption ioo : item.itemOptions) {
                        ...
                        if ((ioo.optionTemplate.id == 224 || ioo.optionTemplate.id == 225
                              || ioo.optionTemplate.id == 226) && (ioo.optionTemplate.id == check)) {
                           if (check == 224) {
                              ioo.param += lvchiso;
                           } else {
               ...
               Service.getInstance().sendThongBao(player,
                     "Không đủ linh hoả. Nâng cấp độ này cần " + cap + " linh hoả");
               return;
            }
         }
      }
   ...
   private void tayphapsu(Player player) {
      if (InventoryService.gI().getCountEmptyBag(player) > 0) {
         if (!player.combineNew.itemsCombine.isEmpty()) {
            ...
            for (ItemOption io : item.itemOptions) {
               if (io.optionTemplate.id == 229 || io.optionTemplate.id == 230 || io.optionTemplate.id == 231
                     || io.optionTemplate.id == 232 || io.optionTemplate.id == 233) {
                  optionStar = io;
                  break;
               }
            }
            ...
            if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()
               ...
               if (optionStar == null) {
                  ...
                  if (item.itemOptions != null) {
                     ...
                     while (iterator.hasNext()) {
                        ItemOption ioo = iterator.next();
                        if (ioo.optionTemplate.id == 229 || ioo.optionTemplate.id == 230 || ioo.optionTemplate.id == 231
                              || ioo.optionTemplate.id == 232 || ioo.optionTemplate.id == 233) {
                           iterator.remove();
                        }
                     }
   ...
   private boolean isTrangBiTaySPL(Item item) {
      if (item != null && item.isNotNullItem()) {
         if ((item.template.type < 5 || item.template.type == 5 || item.template.id == 1558 || item.template.id == 1570
               || item.template.id == 1571 || item.template.id == 1572)) {// && !item.isTrangBiHSD()
            return true;
         } else {
            return false;
         }
