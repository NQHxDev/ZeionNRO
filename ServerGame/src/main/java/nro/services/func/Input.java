...
public class Input {
                  ...
                  try {
                     if (pBuffItem != null) {
                        if ((idDhieu >= 1 && idDhieu <= 3) && SoNgay > 0) {
                           String txtBuff = player.name
                                 + " vừa Tặng Danh hiệu " + (idDhieu == 1 ? "Đại Thần "
                                       : idDhieu == 2
                                             ? "Cần Thủ "
                                             : idDhieu == 3 ? "Tuổi Thơ " : "Thợ Ngọc")
                                 + SoNgay + " Ngày cho người chơi : " + pBuffItem.name;
                           switch (idDhieu) {
                              case 1:
                                 if (pBuffItem.lastTimeTitle1 == 0) {
                                    pBuffItem.lastTimeTitle1 += System.currentTimeMillis()
                                          + (1000 * 60 * 60 * 24 * SoNgay);
                                 } else {
                              ...
                              case 2:
                                 if (pBuffItem.lastTimeTitle2 == 0) {
                                    pBuffItem.lastTimeTitle2 += System.currentTimeMillis()
                                          + (1000 * 60 * 60 * 24 * SoNgay);
                                 } else {
                              ...
                              case 3:
                                 if (pBuffItem.lastTimeTitle3 == 0) {
                                    pBuffItem.lastTimeTitle3 += System.currentTimeMillis()
                                          + (1000 * 60 * 60 * 24 * SoNgay);
                                 } else {
                              ...
                              case 4:
                                 if (pBuffItem.lastTimeTitle4 == 0) {
                                    pBuffItem.lastTimeTitle4 += System.currentTimeMillis()
                                          + (1000 * 60 * 60 * 24 * SoNgay);
                                 } else {
               ...
               try {
                  if (sotvxiu1 >= 1000 && sotvxiu1 <= 100000) {
                     if (player.inventory.ruby >= sotvxiu1) {
                        ...
                        Service.getInstance().sendThongBao(player,
                              "Bạn đã đặt " + Util.format(sotvxiu1) + " Hồng ngọc vào TÀI");
               ...
               try {
                  if (sotvxiu2 >= 1000 && sotvxiu2 <= 100000) {
                     if (player.inventory.ruby >= sotvxiu2) {
                        ...
                        Service.getInstance().sendThongBao(player,
                              "Bạn đã đặt " + Util.format(sotvxiu2) + " Hồng ngọc vào XỈU");
            ...
            case QUY_DOI_COIN:
               ...
               if (goldTrade % 1000 == 0) {
                  if (goldTrade <= 0 || goldTrade >= 1000001) {
                     ...
                     InventoryService.gI().sendItemBags(player);
                     Service.getInstance().sendThongBao(player,
                           "Bạn nhận được " + Util.format((kmnap * (goldTrade / 2)))
                                 + " " + thoiVang.template.name);
                  } else {
                     Service.getInstance().sendThongBao(player,
                           "|7|Số Coin của bạn là " + player.getSession().vnd + " không đủ để quy "
                                 + " đổi " + (goldTrade / 2) + " Xu Bạc" + " " + "bạn cần thêm "
            ...
            case QUY_DOI_COIN_1:
               ...
               if (goldTrade1 % 1000 == 0) {
                  if (goldTrade1 <= 0 || goldTrade1 >= 1000001) {
                     ...
                     InventoryService.gI().sendItemBags(player);
                     Service.getInstance().sendThongBao(player,
                           "Bạn nhận được " + Util.format((kmnap1 * (goldTrade1 / 10)))
                                 + " " + thoiVang.template.name);
                  } else {
                     Service.getInstance().sendThongBao(player,
                           "|7|Số Coin của bạn là " + player.getSession().vnd + " không đủ để quy "
                                 + " đổi " + (goldTrade1 / 10) + " Xu vàng" + " " + "bạn cần thêm "
                                 + (player.getSession().vnd - goldTrade1));
                  }
               } else {
                  Service.getInstance().sendThongBao(player, "|7|Số Coin nhập phải là bội số của 1000");
               }
            ...
            case QUY_DOI_COIN_2:
               ...
               if (goldTrade2 % 1000 == 0) {
                  if (goldTrade2 <= 0 || goldTrade2 >= 1000001) {
                     ...
                     Service.getInstance().sendThongBao(player,
                           "|7|Số Coin của bạn là " + player.getSession().vnd + " không đủ để quy "
                                 + " đổi " + (goldTrade2) + " thỏi vàng" + " " + "bạn cần thêm "
                                 + (player.getSession().vnd - goldTrade2));
                  }
               } else {
                  Service.getInstance().sendThongBao(player, "|7|Số Coin nhập phải là bội số của 1000");
               }
            ...
            case TAI_CLIENT: {
               ...
               try {
                  ...
                  if (itemCuoc != null && itemCuoc.quantity >= sotien_tai) {
                     ...
                     Service.getInstance().sendThongBao(player, "Bạn đã đặt " + Util.format(sotien_tai) + " "
                           + GameDuDoan.NAME_ITEM_CUOC + " vào " + GameDuDoan.LON);
                     ...
                     Service.getInstance().sendThongBao(player,
                           "Bạn không đủ " + GameDuDoan.NAME_ITEM_CUOC + " để chơi.");
                  }
               } catch (Exception e) {
                  e.printStackTrace();
                  Service.getInstance().sendThongBao(player, "Lỗi.");
            ...
            case XIU_CLIENT: {
               ...
               try {
                  ...
                  if (itemCuoc != null && itemCuoc.quantity >= sovang_xiu) {
                     ...
                     Service.getInstance().sendThongBao(player, "Bạn đã đặt " + Util.format(sovang_xiu) + " "
                           + GameDuDoan.NAME_ITEM_CUOC + " vào " + GameDuDoan.NHO);
   ...
   public void createFormBuffItemVip(Player pl) {
      createForm(pl, BUFF_ITEM_OPTION, "BUFF VIP", new SubInput("Tên người chơi", ANY), new SubInput("Id Item", ANY),
            new SubInput("Chuỗi option vd : 50-20;30-1", ANY), new SubInput("Số lượng", ANY));
   }
   ...
   public void ChonSo(Player pl) {
      createForm(pl, CHON_SO_MAY_MAN, "Hãy chọn 1 số từ: 0 đến 99 giá 1.000 Hồng ngọc",
            new SubInput("Số bạn chọn", ANY));// ????
   }

   public void input_Tai(Player pl) {
      createForm(pl, TAI_CLIENT, "Chọn số " + GameDuDoan.NAME_ITEM_CUOC + " đặt " + GameDuDoan.LON,
            new SubInput("Số " + GameDuDoan.NAME_ITEM_CUOC + " cược", ANY));// ????
   }

   public void input_Xiu(Player pl) {
      createForm(pl, XIU_CLIENT, "Chọn số " + GameDuDoan.NAME_ITEM_CUOC + " đặt " + GameDuDoan.NHO,
            new SubInput("Số " + GameDuDoan.NAME_ITEM_CUOC + " cược", ANY));// ????
   }
