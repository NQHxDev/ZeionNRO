...
public class EffectSkillService {
   ...
   // biến hình
   public void setisChoido(Player player) {
      // try {
      // Thread.sleep(1000);
      // } catch (InterruptedException ex) {
      // Logger.getLogger(EffectSkillService.class.getName()).log(Level.SEVERE, null,
      // ex);
      // }
      player.itemTimesieucap.isChoido = true;
      player.itemTimesieucap.lasttimeChoido = System.currentTimeMillis();
   }

   public void removeChoido(Player player) {
      player.itemTimesieucap.isChoido = false;
      if (player.nPoint.hp > player.nPoint.hpMax) {
         player.nPoint.setHp(player.nPoint.hpMax);
      }
      // sendEffectEndBienHinh(player);
   ...
   public void sendEffectbienhinhSc(Player player) {
      Skill skill1 = SkillUtil.getSkillbyId(player, Skill.BIEN_HINH_TD);
      Skill skill2 = SkillUtil.getSkillbyId(player, Skill.BIEN_HINH_XD);
      Skill skill3 = SkillUtil.getSkillbyId(player, Skill.BIEN_HINH_NM);
      if (skill1 == null && skill2 == null && skill3 == null) {
         Service.getInstance().sendThongBao(player, "|7|Lỗi Skill!!!");
         return;
      }
      Message msg;
      try {
   ...
   public void setBienHinhSc(Player player) {
      try {
         Thread.sleep(1000);
      ...
      boolean lastLevel = player.effectSkill.levelBienHinhSc >= skillLevel - 1;

      player.effectSkill.isBienHinhSc = true;
      player.effectSkill.levelBienHinhSc = Math.min(skillLevel, player.effectSkill.levelBienHinhSc + 1);
      player.effectSkill.timeBienHinhSc = SkillUtil.getTimeBienHinhSc(lastLevel, template.coolDown);
      player.effectSkill.lastTimeBienHinhSc = System.currentTimeMillis();

      if (!lastLevel && template.coolDown > 0) {
         player.playerSkill.skillSelect.coolDown = template.coolDown * 5 / 100;
      } else {
   ...
   public void downBienHinh(Player player) {
      player.effectSkill.isBienHinhSc = false;
      player.effectSkill.levelBienHinhSc = 0;

      if (player.nPoint.hp > player.nPoint.hpMax) {
         player.nPoint.setHp(player.nPoint.hpMax);
      }
      sendEffectEndCharge(player);
      sendEffectbienhinhSc(player);
