...
public final class TopService {
   ...
   private static final long[] TIER_TOP = { 1_000_000, 950_000, 870_000, 750_000, 600_000, 550_000, 500_000, 470_000,
         450_000 };
    ...
    * nv = CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(data_task, ',', 2), ',', -1) AS
    * UNSIGNED),
    * nv1 = CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(data_task, ',', 3), ',', -1) AS
    * UNSIGNED),
    * nv2 = CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(data_task, ',', 1), '[', -1) AS
    * UNSIGNED)
    * ORDER BY nv DESC, nv1 DESC, nv2 DESC
    */
    private static final String QUERY_TOP_NV =
        "SELECT p.id, p.name, pt.task_id AS nv, pt.sub_id AS nv1, pt.task_count AS nv2 " +
        "FROM player p " +
    ...
    */
   private static final String QUERY_TOP_VND = "SELECT p.id, p.name, COALESCE(a.tongnap, 0) AS vnd "
         + "FROM player p "
   ...
   public synchronized void showTopSd(Player viewer) {
      try {
         ...
         for (Player pl : online) {
            ...
            String detail = "SM: " + Util.powerToString(pl.nPoint.power)
   ...
   /**
    * Top Nhiệm Vụ – SQL chỉ lấy id/name/nv,nv1,nv2; avatar lấy từ Player nếu
    ...
    * lặp từng dòng:
    * rank(int), pId(int), headID(short), headICON(short=-1), body(short),
    * leg(short),
   ...
   /**
    * Có thể offline: nếu không tìm thấy player online -> head fallback,
    * body/leg=0, detail="Người chơi đang offline"
    */
