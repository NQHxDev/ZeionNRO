   ...
   public boolean openPowerSpeed(Player player) {
      if (player.nPoint.limitPower < NPoint.MAX_LIMIT) {
         ...
         if (!player.isPet) {
            ...
            Service.getInstance().sendThongBao(((Pet) player).master,
                  "Giới hạn sức mạnh của đệ tử đã được tăng lên 1 bậc");
         }
         return true;
         // } else {
         // if (!player.isPet) {
         // Service.getInstance().sendThongBao(player, "Sức mạnh của bạn không đủ để thực
         // hiện");
         // } else {
         // Service.getInstance().sendThongBao(((Pet) player).master, "Sức mạnh của đệ tử
