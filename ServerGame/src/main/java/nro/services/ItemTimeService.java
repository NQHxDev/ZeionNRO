...
public class ItemTimeService {
   ...
   public void sendAllItemTime(Player player) {
      ...
      if (player.itemTime.isOpenPower) {
         sendItemTime(player, 3783,
               (int) ((TIME_OPEN_POWER - (System.currentTimeMillis() - player.itemTime.lastTimeOpenPower)) / 1000));
      }
      ...
      if (player.itemTimesieucap.isDaNgucTu) {
         sendItemTime(player, 11274,
               (int) ((TIME_ITEM_SC_10P - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeDaNgucTu))
                     / 1000));
      }
      if (player.itemTimesieucap.isDuoikhi) {
         sendItemTime(player, 5072,
               (int) ((TIME_ITEM_SC_10P - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeDuoikhi))
                     / 1000));
      }
      if (player.itemTimesieucap.isUseCaRot) {
         sendItemTime(player, 16128,
               (int) ((TIME_ITEM_SC_10P - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeCaRot)) / 1000));
      }
      if (player.itemTimesieucap.isKeo) {
         sendItemTime(player, 8243,
               (int) ((TIME_ITEM_SC_30P - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeKeo)) / 1000));
      }
      if (player.itemTimesieucap.isUseXiMuoi) {
         sendItemTime(player, 10904,
               (int) ((TIME_ITEM_SC_10P - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeUseXiMuoi))
                     / 1000));
      }
      if (player.itemTimesieucap.isUseTrungThu) {
         sendItemTime(player, player.itemTimesieucap.iconBanh,
               (int) ((TIME_TRUNGTHU - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeUseBanh)) / 1000));
      }
      ...
      if (player.itemTimesieucap.isBienhinhSc) {
         ...
         sendItemTime(player, (iconLvFirst + lv),
               (int) ((TIME_ITEM_SC_10P - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeBienhinhSc))
                     / 1000));
      }
      // if (player.itemTimesieucap.isRongBang) {
      // sendItemTime(player, 8579, (int) ((TIME_ITEM_SC_30P -
   ...
   public void sendTextRongBang(Player player) {
      if (player.itemTimesieucap.isRongBang) {
         int secondsLeft = (int) ((TIME_ITEM_SC_30P
               - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeRongBang)) / 1000);
   ...
   public void sendTextRongSieuCap(Player player) {
      if (player.itemTimesieucap.isRongSieuCap) {
         int secondsLeft = (int) ((TIME_ITEM_SC_30P
               - (System.currentTimeMillis() - player.itemTimesieucap.lastTimeRongSieuCap)) / 1000);
