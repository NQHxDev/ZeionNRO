   ...
   private void openListFriend(Player player) {
      ...
      try {
         ...
         for (Friend f : player.friends) {
            ...
            msg.writer().writeUTF(Util.powerToStringnew(f.power));
         }
   ...
   private void openListEnemy(Player player) {
      ...
      try {
         ...
         for (Enemy e : player.enemies) {
            ...
            msg.writer().writeUTF(Util.powerToStringnew(e.power));
         ...
         if (pl != null) {
            if (player.isAdmin() || player.nPoint.teleport) {
               ...
               if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
                  if (MapService.gI().isMapDoanhTrai(mapid) || (mapid >= 141 && mapid <= 143) || mapid == 45
                        || mapid == 46 || mapid == 47) {
                     Service.getInstance().sendThongBao(player, "Không thể thực hiện");
                     return;
                  }
                  if (player.isAdmin() || (!pl.zone.isFullPlayer() && !(mapid >= 85 && mapid <= 91)
                        ...
                        && !(mapid >= 160 && mapid <= 163) && !(MapService.gI().isMapMabuWar(mapid)))) {
                     ChangeMapService.gI().changeMapYardrat(player, pl.zone, pl.location.x + Util.nextInt(-5, 5),
                           pl.location.y);
                  } else {
                     Service.getInstance().sendThongBao(player, "Không thể thực hiện");
                  }
