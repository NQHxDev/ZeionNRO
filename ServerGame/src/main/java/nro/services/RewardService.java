...
public class RewardService {
   ...
   // id option set kich hoat (tên set, hiệu ứng set, tỉ lệ, type tỉ lệ)
   private static final int[][][] ACTIVATION_SET_NO = { { { 129, 141 }, { 127, 139 }, { 128, 140 } }, // songoku - thien
                                                                                                      // xin hang -
                                                                                                      // kirin
   ...
   public List<ItemMap> getRewardItems(Player player, Mob mob, int x, int yEnd) {
      ...
      // ============================================ TẤT CẢ CÁC MAP FAM CÓ TỈ LỆ RƠI
      // THỎI VÀNG
      // ============================================================================
      ...
      if (mob.zone.map.mapId == 156) {// 10% rơi 1 -> 3 mảnh vỡ bt cấp 2
         if (check == true ? tile : Util.isTrue(1, 10)) {
            ItemMap itemMap = new ItemMap(mob.zone, 1773, Util.nextInt(1, 3), x + Util.nextInt(-50, 50), yEnd,
                  player.id);
            list.add(itemMap);
         }
      }
      ...
      if (mob.zone.map.mapId == 157) {// 5% rơi 1 -> 3 mảnh vỡ bt cấp 2
         if (check == true ? tile : Util.isTrue(1, 20)) {
            ItemMap itemMap = new ItemMap(mob.zone, 1774, Util.nextInt(1, 3), x + Util.nextInt(-50, 50), yEnd,
                  player.id);
            list.add(itemMap);
         }
      }
      ...
      if (mob.zone.map.mapId == 158) {// 2% rơi 1 -> 3 mảnh vỡ bt cấp 2
         if (check == true ? tile : Util.isTrue(1, 50)) {
            ItemMap itemMap = new ItemMap(mob.zone, 1775, Util.nextInt(1, 3), x + Util.nextInt(-50, 50), yEnd,
                  player.id);
            list.add(itemMap);
         }
      }
      ...
      if (mob.zone.map.mapId == 214) {// 20% Rơi ngẫu nhiên 1->20 thỏi vang
         if (check == true ? tile : Util.isTrue(1, 5)) {
            ItemMap itemMap = new ItemMap(mob.zone, 457, Util.nextInt(1, 20), x + Util.nextInt(-50, 50), yEnd,
                  player.id);
            list.add(itemMap);
         }
      }
      ...
      if (mobReward != null) {
         ...
         if (itemSize > 0) {
               ...
               if (ir.forAllGender || ItemService.gI().getTemplate(ir.tempId).gender == player.gender
                  ...
                  if (check == true ? tile : Util.isTrue(ir.ratio, ir.typeRatio)) {
                     ...
                     switch (itemMap.itemTemplate.type) {
                        ...
                        case 4:
                           ...
                           initStarOption(itemMap,
                                 new RatioStar[] { new RatioStar((byte) 1, 20, 100), new RatioStar((byte) 2, 10, 100),
                                       new RatioStar((byte) 3, 5, 100), new RatioStar((byte) 4, 3, 200),
                                       new RatioStar((byte) 5, 2, 200), new RatioStar((byte) 6, 1, 200),
                     ...
                     if (itemMap.itemTemplate.id >= 555 && itemMap.itemTemplate.id <= 567
                           || itemMap.itemTemplate.id == 2013
                           || itemMap.itemTemplate.id >= 1048 && itemMap.itemTemplate.id <= 1062
                           || itemMap.itemTemplate.id == 547
                           || itemMap.itemTemplate.id >= 650 && itemMap.itemTemplate.id <= 662
                           || itemMap.itemTemplate.id == 548 || itemMap.itemTemplate.id == 457) {
            ...
            // ItemMap itemMap = new ItemMap(mob.zone, ConstItem.XU_VANG, 1, x, yEnd,
            // player.id);
            ...
            // ItemMap itemMap = new ItemMap(mob.zone, 2094, Util.nextInt(1, 3), x, yEnd,
            // player.id);
            // list.add(itemMap);
            // }
            // }
