...
public class InventoryService {
   ...
   public boolean addItemList(List<Item> items, Item item, int maxQuantity) {
      ...
      if (item.template.isUpToUp) {
         for (Item it : items) {
            if (!it.isNotNullItem() || it.template.id != item.template.id
                  || (hasOptionTemplateId(it, 73) && hasOptionTemplateId(item, 30))) {
               continue;
            }
            if (!it.isNotNullItem() || it.template.id != item.template.id
                  || (hasOptionTemplateId(it, 30) && hasOptionTemplateId(item, 73))) {
               continue;
            }
            if (!it.isNotNullItem() || it.template.id != item.template.id
            ...
            if (item.template.id == 457 || item.template.type == 27 || item.template.type == 99
                  || item.template.type == 14 || item.template.type == 6
                  || item.template.type == 12 || item.template.type == 29 || item.template.type == 33
                  || item.template.type == 31 || item.template.type == 30) {
   ...
   public void throwItem(Player player, int where, int index) {
      ...
      if (where == 0) {
         ...
         if (index >= 0 && index <= player.inventory.itemsBag.size()) {
            ...
            if (itemThrow.isNotNullItem()) {
               if (itemThrow.template.id != 457 && itemThrow.template.id != 1400 && itemThrow.template.id != 2013
                     && itemThrow.template.id != 992) {
   ...
   public void itemBagToPetBody(Player player, int index) {
      if (player.pet != null && player.pet.nPoint.power >= 699696) {
         ...
         Item item = player.inventory.itemsBag.get(index);
         if (item.template.id == 1326 || item.template.id == 1325 || item.template.id == 619 || item.template.id == 620
               || item.template.id == 621 || item.template.id == 1772
               || item.template.id == 547 || item.template.id == 548
               || (item.template.type >= 0 && item.template.type <= 5) || item.template.type == 32
               || item.template.type == 23 || item.template.type == 35) {
            if (item.isNotNullItem()) {
   ...
   public void itemBoxToBodyOrBag(Player player, int index) {
      ...
      if (item.isNotNullItem()) {
         boolean done = false;
         // if (item.template.type >= 0 && item.template.type <= 4 || item.template.type
         // == 32) {
         // Item itemBody = player.inventory.itemsBody.get(item.template.type == 32 ? 6 :
         // item.template.type);
         ...
         // player.inventory.itemsBody.set(item.template.type == 32 ? 6 :
         // item.template.type, item);
