 ...
 *
 */
public class GiftcodePlayer {

    private static GiftcodePlayer i;

    private GiftcodePlayer() {

    }

    public static GiftcodePlayer gI() {
        if (i == null) {
            i = new GiftcodePlayer();
        }
        return i;
    }
    ...
    public void use(Player player, String code) {
        ...
        try {
            PreparedStatement stmt = DBService.gI().getConnectionForGame().prepareStatement("SELECT * FROM `member_gift` WHERE `coded` like ? AND (expires_atd IS NULL OR expires_atd > now()) LIMIT 1;",
            ...
            try {
                ...
                int id = res.getInt("idd");
                byte status = res.getByte("statusd");
                byte type = res.getByte("typed");
                boolean active = res.getBoolean("actived");
                int plId = res.getInt("player_idd");

                if (status == 1) {
                    Service.getInstance().sendThongBaoOK(player, "Mã quà tặng đã hết hạn");
                    return;
                } else if (type == 1 && isUsedGiftCode((int) player.id, id)) {
                    Service.getInstance().sendThongBaoOK(player, "Giftcode đã được sử dụng");
                    return;
                    ...
                    return;
                } else if (plId != player.id) {
                    Service.getInstance().sendThongBao(player, "Bạn không thuộc đặc quyền nhập code này");
                    return;
                }

                int gold = res.getInt("goldd");
                int gem = res.getInt("gemd");
                int ruby = res.getInt("rubyd");

                JSONArray arrItem = new JSONArray(res.getString("itemsd"));
    ...
    private boolean isUsedGiftCode(int playerID, int giftCodeId) {
        try {
            PreparedStatement stmt = DBService.gI().getConnectionForGame().prepareStatement("SELECT * FROM `member_gift_lichsu` WHERE `gift_code_idd` = ? AND `player_idd` = ? LIMIT 1;", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ...
    private void addUsedGiftCode(int playerID, int giftCodeId, String code) {
        try {
            Timestamp timestamp = new Timestamp(System.currentTimeMillis());
            PreparedStatement stmt = DBService.gI().getConnectionForGame().prepareStatement("INSERT INTO `member_gift_lichsu`(`player_idd`, `gift_code_idd`, `coded`, `created_atd`) VALUES (?, ?, ?, ?)");
