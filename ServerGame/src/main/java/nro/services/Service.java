...
public class Service {
   ...
   public void chat(Player player, String text) {
      ...
      if (player.getSession() != null && player.isAdmin()) {
         ...
         // Service.getInstance().sendThongBao(player, player.location.x + " - " +
         // player.location.y);
         // return;
      ...
      // return;
      // } else if (player.lastTimeTitle1 != 0 && player.lastTimeTitle2 != 0
      // && player.lastTimeTitle3 != 0 && player.lastTimeTitle4 != 0 &&
   ...
   public void chatGlobal(Player pl, String text) {
      if (pl.inventory.getGold() >= 2000000000) {
         if (pl.isAdmin() || Util.canDoWithTime(pl.lastTimeChatGlobal, 18000)) {
            ...
            sendThongBao(pl,
                  "Không thể chat thế giới lúc này, vui lòng đợi " + TimeUtil.getTimeLeft(pl.lastTimeChatGlobal, 120));
         }
      } else {
   ...
   public void chooseFlag(Player pl, int index) {
      if (Util.canDoWithTime(pl.lastTimeChangeFlag, 60000)) {
         if (!MapService.gI().isMapBlackBallWar(pl.zone.map.mapId) && !MapService.gI().isMapMabuWar(pl.zone.map.mapId)
               && !pl.isHoldBlackBall) {
            changeFlag(pl, index);
         } else {
         ...
         sendThongBao(pl,
               "Không thể đổi cờ lúc này! Vui lòng đợi " + TimeUtil.getTimeLeft(pl.lastTimeChangeFlag, 60) + " nữa!");
      }
   }
   ...
   public void showTopPower(Player player, int top) {
      ...
      try {
         ...
         msg.writer().writeUTF(top == TOP_SUCMANH ? "Top Sức Mạnh"
               : top == TOP_DETU ? "Top Đệ tử" : top == TOP_NHIEMVU ? "Top Nhiệm vụ" : "Top Donate");
         ...
         for (int i = 0; i < list.size(); i++) {
            ...
            if (top == TOP_DETU && pl.pet != null) {
               msg.writer()
                     .writeUTF("Loại: " + (pl.pet.typePet == 0 ? "Đệ tử Thường"
                           : pl.pet.typePet == 1 ? "Mabu"
                                 : pl.pet.typePet == 2 ? "Berus"
                                       : pl.pet.typePet == 3 ? "Zeno"
                                             : pl.pet.typePet == 4 ? "Kaido"
                                                   : pl.pet.typePet == 5 ? "Itachi"
                                                         : pl.pet.typePet == 6 ? "Tiên hắc ám"
                                                               : pl.pet.typePet == 7 ? "Ngộ Không" : "Long Đế"));
            } else {
               msg.writer().writeUTF(Client.gI().getPlayer(pl.id) != null ? "Online" : "Chưa Online");
            }
            msg.writer().writeUTF(top == TOP_SUCMANH
                  ? "Sức mạnh: " + Util.powerToString(pl.nPoint.power) + "\n Chuyển sinh: " + pl.chuyensinh
   ...
   public void showTopSieuHang(Player player) {
      ...
      try {
         ...
         for (int i = 0; i < list.size(); i++) {
            ...
            msg.writer().writeShort(pl.getLeg());
            msg.writer().writeUTF(pl.name);
            msg.writer().writeUTF(pl.rankSieuHang == 1 ? ("+500Tr Vàng/ngày")
                  : (pl.rankSieuHang > 1 && pl.rankSieuHang < 10) ? ("+50Tr Vàng/ngày") : "");
   ...
   public void Mabu14hAttack(Boss mabu, Player plAttack, int x, int y, byte skillId) {
      ...
      try {
         ...
         if (skillId == 1) {
            msg.writer().writeByte(1);
            double dame = Util.DoubleGioihan(plAttack.injured(mabu,
                  (int) (mabu.nPoint.getDameAttack(false) * (skillId == 1 ? 1.5 : 1)), false, false));
