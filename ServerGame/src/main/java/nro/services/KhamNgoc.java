...
public class KhamNgoc {
   ...
   public void NangCapKhamNgoc(Player pl, byte nro) {
      ...
      if (item == null || item.quantity < max_quatity) {
         Service.getInstance().sendThongBao(pl, "Không đủ nguyên liệu. Còn thiếu "
   ...
   // int levelBeforeMax = KHAM_NGOC.get(nro - 1).khamNgocTemplates.size();
   // if (levelBefore == - 1) {
   // Service.getInstance().sendThongBao(pl, "Vui lòng kích hoạt Ngọc rồng " + nro
   // + " sao trước");
   // return;
   // }
   // if (levelBefore < (levelBeforeMax - 1)) {
   // Service.getInstance().sendThongBao(pl, "Vui lòng Nâng Ngọc rồng " + nro + "
   // sao đến cấp tối đa trước");
   // return;
   // }
   // }
   // int idTemp = khamNgoc.khamNgocTemplates.get(level + 1).tempId;
   ...
   // if (item == null || item.quantity < max_quatity) {
   // Service.getInstance().sendThongBao(pl, "Không đủ nguyên liệu. Còn thiếu " +
   // (item == null ? max_quatity : (max_quatity - item.quantity)) + " " +
   // it.template.name);
   // return;
   // }
   ...
   // Service.getInstance().sendThongBao(pl, "|2|Kích hoạt thành công Ngọc rồng " +
   // (nro + 1) + " sao");
   // } else {
   // Service.getInstance().sendThongBao(pl, "|2|Nâng thành công Ngọc rồng " + (nro
   // + 1) + " sao lên Cấp " + manager.levelNro);
   // }
   // }
