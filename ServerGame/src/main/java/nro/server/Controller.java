...
public class Controller {
   ...
   // private static final String CLIENT_SHARED_SECRET =
   // "NRO_SERVERGAME_NEW_14_1_2026";// Helper: hex -> bytes
   ...
   // Service.getInstance().sendThongBaoOK(session,
   // note != null ? note : (ok ? "Đăng ký thành công! Vui lòng đăng nhập." : "Đăng
   // ký thất bại."));
   // }
   // }
   ...
   // ==================
   // private void handleRegisterWithCaptcha(Session session, String user, String
   // pass, String answer, String token) {
   // try {
   // // 1) Kiểm tra cơ bản
   ...
   // // sendPreloginCase5(session, false, "Tài khoản chỉ gồm a-z, 0-9, ., _ (5-20
   // ký
   // // tự)");
   // // return;
   // // }
   ...
   // // Ví dụ: int accId = AccountDAO.createAccount(user,
      ...
      try {
         ...
         try (Connection con = DBService.gI().getConnectionForSaveData();
            ...
            try (ResultSet rs = ps.executeQuery()) {
               if (rs.next()) {
                  Service.getInstance().sendThongBaoOK(session,
                        "Tên nhân vật đã tồn tại hoặc tài khoản đã có nhân vật");
                  return;
               }
            }
