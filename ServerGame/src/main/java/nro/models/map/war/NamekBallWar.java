...
public class NamekBallWar {
   ...
   public Player[] holders = new Player[7];
   ...
   public void pickBall(Player player, NamekBall item) {
      ...
      item.isHolding = true;
      holders[item.index] = player;
      Service.getInstance().sendFlagBag(player);
      item.reAppearItem();
      item.holderName = player.name;
      Service.getInstance().sendThongBao(player, "Bạn đang giữ viên ngọc rồng Namek " + (item.index + 1) + " sao");
   }
      ...
      for (Player holder : holders) {
         if (holder == null || holder.clan.id != clanID) {
            Service.getInstance().sendThongBao(player,
                  "Cần tập hợp đủ 7 viên ngọc rồng mới có thể triệu hồi rồng thần");
            return;
         }
      }
      NamekBall oneStar = NamekBallManager.gI().findByID(ConstItem.NGOC_RONG_NAMEK_1_SAO);
      if (!oneStar.isCleaning) {
         oneStar.isCleaning = true;
         oneStar.cleaningTime = 9 * 60;
         npc.npcChat(player,
               "Hãy đợi 9 phút em lau sạch ngọc rồng,trong thời gian lau ngọc nếu một trong các viên ngọc bị rơi sẽ làm mới thời gian lau");
         return;
      }
      if (oneStar.isCleaning && oneStar.cleaningTime == 0) {
