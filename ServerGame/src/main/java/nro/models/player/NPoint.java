...
public class NPoint {
   ...
   private void setHpMax() {
      ...
      if (this.player.isPet && this.player.PorataVIP == true
      ...
      if (this.player.itemTimesieucap != null && this.player.itemTimesieucap.isUseTrungThu
            && this.player.itemTimesieucap.iconBanh == 4042) {
         this.hpMax += calPercent(hpMax, 10);
      }
      if (this.player.itemTimesieucap != null && this.player.itemTimesieucap.isUseTrungThu
            && this.player.itemTimesieucap.iconBanh == 4043) {
         this.hpMax += calPercent(hpMax, 20);
      }
      if (this.player.itemTimesieucap != null && this.player.itemTimesieucap.isUseTrungThu
            && this.player.itemTimesieucap.iconBanh == 4125) {
         this.hpMax += calPercent(hpMax, 30);
      }
      if (this.player.itemTimesieucap != null && this.player.itemTimesieucap.isUseTrungThu
            && this.player.itemTimesieucap.iconBanh == 4126) {
   ...
   private void setMpMax() {
      ...
      // }
      // if (this.player.effectFlagBag.useHoaVang ||
      // this.player.effectFlagBag.useHoaHong) {
      // this.mpMax += calPercent(this.mpMax, 20);
      // }
      if (!player.isBoss) {
      ...
      if (player.isPl()) {
         if (player.inventory.haveOption(player.inventory.itemsBody, 5, 159)) {
            if (Util.canDoWithTime(player.lastTimeUseOption, 60000)
               ...
               Service.getInstance().sendThongBao(player, "|1|Bạn vừa gây ra x"
                     + player.inventory.getParam(player.inventory.itemsBody.get(5), 159) + " Sát thương Chưởng");
            }
         }
      }
         ...
         if (player.effectSkill.isCharging && player.effectSkill.countCharging < 10) {
            ...
            if (player.effectSkill.isCharging && !player.isDie() && !player.effectSkill.isHaveEffectSkill()
               ...
               if (player.effectSkill.countCharging % 3 == 0) {
                  Service.getInstance().chat(player,
                        "Phục hồi năng lượng " + Util.LongGioihan(getCurrPercentHP()) + "%");
               }
            } else {
   ...
   // BigInteger result =
   // BigInteger.valueOf(base).multiply(BigInteger.valueOf(multiplier));
      ...
      if (this.player.active_ruong_suu_tam == 1) {
         for (int i = 0; i < RuongSuuTam.listCaiTrang.size(); i++) {
            for (int k = 0; k < this.player.ruongSuuTam.RuongCaiTrang.size(); k++) {
               ...
               if (template.template != null && listRuong.template != null
                     && listRuong.template.id == template.template.id) {
