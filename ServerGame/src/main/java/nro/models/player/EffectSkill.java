...
public class EffectSkill {
   ...
   public void update() {
      ...
      if (player.isBoss && player.playerSkill.prepareQCKK
            && Util.canDoWithTime(player.playerSkill.lastTimeUseQCKK, 4000)) {
      ...
      if (player.isPl()) {
         ...
         if (this.isTaskHoldMabu == 2 && player.zone.map.mapId == 128
               && Util.canDoWithTime(this.lastTimeHoldMabu, 600)) {
            if (!this.isShielding) {
