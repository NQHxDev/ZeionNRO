...
public class SuperBroly extends Broly {
   ...
   public void rewards(Player pl) {
      ...
      // Service.getInstance().sendThongBaoOK(pl, "Chúc mừng bạn vừa tiêu diệt boss "
      // + this.name + " nhận được đệ tử");
      // ServerNotify.gI().notify(pl.name + " vừa tiêu diệt " + this.name + "tại " +
      // this.zone.map.mapName + " khu vực " + this.zone.zoneId);
      // } else {
      // ServerNotify.gI().notify(pl.name + " vừa phá boss " + this.name + " tại " +
      // this.zone.map.mapName + " khu vực " + this.zone.zoneId);
      // }
   }
