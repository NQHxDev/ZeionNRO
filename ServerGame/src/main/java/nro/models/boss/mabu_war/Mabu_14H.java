            ...
            if (!useSpecialSkill()) {
               if (this.id != BossFactory.MABU_MAP) {
                  ...
                  switch (idSkill) {
                     case 0:
                        if (pl.isPl()
                              && MapService.gI().getZoneJoinByMapIdAndZoneId(this, 128, zoneHold).getNumOfPlayers() < 4
                              && Util.canDoWithTime(pl.effectSkill.lastTimeHoldMabu, 9000) && pl.zone.map.mapId != 128
                              && pl.effectSkill.isTaskHoldMabu <= 0
                              && Util.canDoWithTime(this.lastTimeUseSpeacialSkill, 12000)) {
                     ...
                     case 1:
                        ...
                        break;
                     default:
                        if (Util.canDoWithTime(lastTimeUseSpeacialSkill, 7000) && !list.isEmpty()
                              && Util.isTrue(40, 100)) {
                           Service.getInstance().Mabu14hAttack(this, pl,
                                 this.location.x + ((pl.location.x <= this.location.x) ? -30 : 30), this.location.y,
                                 (byte) 0);
                        }
                        break;
                  }
