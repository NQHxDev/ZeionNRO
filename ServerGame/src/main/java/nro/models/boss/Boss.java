...
public abstract class Boss extends Player implements BossInterface {
   ...
   public void dropItemReward(int tempId, int playerId, int... quantity) {
      if (!this.zone.map.isMapOffline && this.zone.map.type == ConstMap.MAP_NORMAL) {
         ...
         ItemMap itemMap = new ItemMap(this.zone, tempId, (quantity != null && quantity.length == 1) ? quantity[0] : 1,
               x, y, playerId);
   ...
   public void tileRoiCT_DeTu_RandomChiSo(Player pl, int caitrang, int tile, int sd1, int sd2, int hpki1, int hpki2,
         int tnsm1, int tnsm2, int tlvinhvien, int hansudungmax) {
      ...
      if (Util.isTrue(tile, 100)) {
         ...
         RewardService.gI().initBaseOptionClothes(itemMap1.itemTemplate.id, itemMap1.itemTemplate.type,
               itemMap1.options);
         Service.getInstance().dropItemMap(zone, itemMap1);
      }
   }

   public void tileRoiCT_SuPhu_RandomChiSo(Player pl, int caitrang, int tile, int sd1, int sd2, int hpki1, int hpki2,
         int tnsm1, int tnsm2, int tlvinhvien, int hansudungmax) {
      ...
      if (Util.isTrue(tile, 100)) {
         ...
         RewardService.gI().initBaseOptionClothes(itemMap2.itemTemplate.id, itemMap2.itemTemplate.type,
               itemMap2.options);
   ...
   public void tileRoiNhanThanLinh(Player pl, int tile, int tileloSPL, int SoloSPLMax) {
      ...
      if (Util.isTrue(tile, 100)) {
         ...
         RewardService.gI().initBaseOptionClothes(itemMap3.itemTemplate.id, itemMap3.itemTemplate.type,
               itemMap3.options);
   ...
   public void tileRoiDoThanLinh(Player pl, int tile, int tileloSPL, int SoloSPLMax) {
      ...
      if (Util.isTrue(tile, 100)) {
         ...
         RewardService.gI().initBaseOptionClothes(itemMap4.itemTemplate.id, itemMap4.itemTemplate.type,
               itemMap4.options);
   ...
   // public void tileRoiNhanHD(Player pl,int nhanhd, int tile, int tileloSPL, int
   // SoloSPLMax) {
   // ItemMap itemMap5 = null;
   // if (Util.isTrue(tile, 100)) {
   // itemMap5 = ratiItemHuyDiet(zone, nhanhd, 1, this.location.x - 20,
   // this.location.y, pl.id);
   // }
   // RewardService.gI().initStarOption(itemMap5, new RewardService.RatioStar[]{
   // new RewardService.RatioStar((byte) Util.nextInt(1, SoloSPLMax), tileloSPL,
   // 100),});
   ...
   public void tileRoiDoHD(Player pl, int dohd, int tile, int tileloSPL, int SoloSPLMax) {
      if (Util.isTrue(tile, 100)) {
         ItemMap itemMap6 = ratiItemHuyDiet(zone, dohd, 1, this.location.x - 20, this.location.y, pl.id);
         RewardService.gI().initStarOption(itemMap6, new RewardService.RatioStar[] {
               new RewardService.RatioStar((byte) Util.nextInt(1, SoloSPLMax), tileloSPL, 100), });
   ...
   public void tileRoiGangNhanThienSu(Player pl, int tile, int tileloSPL, int SoloSPLMax) {
      if (Util.isTrue(tile, 100)) {
         ...
         ItemMap itemMap7 = ratiItemThienSu(zone, set1[Util.nextInt(0, set1.length - 1)], 1, this.location.x - 20,
               this.location.y, pl.id);
   ...
   public void tileRoiQuanAoThienSu(Player pl, int tile, int tileloSPL, int SoloSPLMax) {
      if (Util.isTrue(tile, 100)) {
         ...
         ItemMap itemMap8 = ratiItemThienSu(zone, set1[Util.nextInt(0, set1.length - 1)], 1, this.location.x - 20,
               this.location.y, pl.id);
   ...
   public static ItemMap ratiItemHuyDiet(Zone zone, int tempId, int quantity, int x, int y, long playerId) {
      ...
      if (ao.contains(tempId)) {
         it.options.add(
               new ItemOption(47, Util.highlightsItem(it.itemTemplate.gender == 2, new Random().nextInt(1001) + 1800))); // áo
                                                                                                                         // từ
                                                                                                                         // 1800-2800
                                                                                                                         // giáp
      }
      if (quan.contains(tempId)) {
         it.options.add(
               new ItemOption(22, Util.highlightsItem(it.itemTemplate.gender == 0, new Random().nextInt(16) + 85))); // hp
                                                                                                                     // 85-100k
      }
      if (gang.contains(tempId)) {
         it.options.add(
               new ItemOption(0, Util.highlightsItem(it.itemTemplate.gender == 2, new Random().nextInt(1500) + 8500))); // 8500-10000
      }
      if (giay.contains(tempId)) {
         it.options.add(
               new ItemOption(23, Util.highlightsItem(it.itemTemplate.gender == 1, new Random().nextInt(11) + 80))); // ki
                                                                                                                     // 80-90k
      }
      if (nhd == tempId) {
   ...
   public static ItemMap ratiItemThienSu(Zone zone, int tempId, int quantity, int x, int y, long playerId) {
      ...
      // áo
      if (ao.contains(tempId)) {
         dots.options.add(new ItemOption(47,
               Util.highlightsItem(dots.itemTemplate.gender == 2, new Random().nextInt(1201) + 2800))); // áo từ
                                                                                                        // 2800-4000
                                                                                                        // giáp
      ...
      if (Util.isTrue(60, 100)) {
         if (quan.contains(tempId)) {
            dots.options.add(new ItemOption(22,
                  Util.highlightsItem(dots.itemTemplate.gender == 0, new Random().nextInt(11) + 120))); // hp 120k-130k
         }
      } else {
         if (quan.contains(tempId)) {
            dots.options.add(new ItemOption(22,
                  Util.highlightsItem(dots.itemTemplate.gender == 0, new Random().nextInt(51) + 130))); // hp 130-180k
                                                                                                        // 15%
            if (Util.isTrue(30, 100)) {
      ...
      if (Util.isTrue(60, 100)) {
         if (gang.contains(tempId)) {
            dots.options.add(new ItemOption(0,
                  Util.highlightsItem(dots.itemTemplate.gender == 2, new Random().nextInt(7000) + 12000))); // 11000-18600
         }
      } else {
         if (gang.contains(tempId)) {
            dots.options.add(new ItemOption(0,
                  Util.highlightsItem(dots.itemTemplate.gender == 2, new Random().nextInt(7000) + 12000))); // gang 15%
                                                                                                            // 12-19k
                                                                                                            // -xayda
                                                                                                            // 12k1
            if (Util.isTrue(30, 100)) {
      ...
      if (Util.isTrue(60, 100)) {
         if (giay.contains(tempId)) {
            dots.options.add(new ItemOption(23,
                  Util.highlightsItem(dots.itemTemplate.gender == 1, new Random().nextInt(21) + 120))); // ki 90-110k
         }
      } else {
         if (giay.contains(tempId)) {
            dots.options.add(new ItemOption(23,
                  Util.highlightsItem(dots.itemTemplate.gender == 1, new Random().nextInt(21) + 130))); // ki 110-130k
            if (Util.isTrue(30, 100)) {
      ...
      if (nhan.contains(tempId)) {
         dots.options.add(
               new ItemOption(14, Util.highlightsItem(dots.itemTemplate.gender == 1, new Random().nextInt(3) + 18))); // nhẫn
                                                                                                                      // 18-20%
         if (Util.isTrue(30, 100)) {
