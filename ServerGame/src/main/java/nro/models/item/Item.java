...
public class Item {
   ...
   public boolean canConsign() {
      ...
      for (ItemOption o : itemOptions) {
         ...
         if (template.id != ConstItem.THOI_VANG && (optionId != 30)
               && (type == 12 || type == 33 || type == 29 || type == 27 || type == 99 || type == 11 || type == 5
                     || (type < 4 && (optionId == 86 || optionId == 87)))) {
            return true;
         }
      }
      return false;
   ...
   public boolean isSKH() {
      for (ItemOption itemOption : itemOptions) {
         if (itemOption.optionTemplate.id >= 127 && itemOption.optionTemplate.id <= 135
               || this.template.id == 1553 && !this.isTrangBiHSD() || this.template.id == 1772) {
            return true;
         }
      }
      return false;
   ...
   public boolean isDaCuongHoa() {
      return ConstItem.DA_CUONG_HOA_IDS.contains((int) this.template.id);
   }
   ...
   public boolean isTrangBiHSD() {
      return InventoryService.gI().hasOptionTemplateId(this, 93)
            || InventoryService.gI().hasOptionTemplateId(this, 199);
   }
