      ...
      if (this.day == -1 || this.day != currDay) {
         ...
         try {
            TranhNgoc.TIME_OPEN = TimeUtil.getTime(
                  ...
                  "dd/MM/yyyy HH:mm:ss");
            TranhNgoc.TIME_CLOSE = TimeUtil.getTime(
                  TimeUtil.getTimeNow("dd/MM/yyyy") + " " + HOUR_CLOSE + ":" + MIN_CLOSE + ":" + 0,
                  "dd/MM/yyyy HH:mm:ss");
            TranhNgoc.TIME_REGISTER = TimeUtil.getTime(
                  TimeUtil.getTimeNow("dd/MM/yyyy") + " " + HOUR_REGISTER + ":" + MIN_REGISTER + ":" + 0,
                  "dd/MM/yyyy HH:mm:ss");
         } catch (Exception e) {
         }
      }
      ...
      try {
         ...
         if (Util.canDoWithTime(player.lastTimeUpdateBallWar, 1000)) {
            ...
            if (player.zone.map.mapId == ConstTranhNgocNamek.MAP_ID) {
               try {
                  if (!isTimeStartWar() || (!player.zone.getPlayersFide().contains(player)
                        && !player.zone.getPlayersCadic().contains(player))) {
                     kickOutOfMap(player);
                  }
               } catch (Exception ex) {
                  ex.printStackTrace();
